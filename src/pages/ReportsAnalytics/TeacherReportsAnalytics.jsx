...
const TeacherReportsAnalytics = () => {
  ...
  const StudentReportModal = () => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`rounded-xl sm:rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto ${
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className={`p-3 sm:p-4 rounded-lg ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}>
              <div className="text-sm text-gray-500">Overall Performance</div>
              <div className="text-xl sm:text-2xl font-bold text-green-600">{selectedStudent?.overall}</div>
            </div>
            <div className={`p-3 sm:p-4 rounded-lg ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}>
              <div className="text-sm text-gray-500">Attendance</div>
              <div className="text-xl sm:text-2xl font-bold text-blue-600">{selectedStudent?.attendance}</div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {selectedStudent?.details.strengths.map((strength, index) => (
                <span key={index} className="px-2 py-1 bg-green-100 text-green-700 rounded-full text-xs sm:text-sm">
                  {strength}
                </span>
              ))}
            </div>
      ...
      <div className={`rounded-xl sm:rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto ${
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          <div className="grid grid-cols-3 gap-4">
            <div className={`p-3 sm:p-4 rounded-lg text-center ${isDarkMode ? 'bg-slate-700' : 'bg-blue-50'}`}>
              <div className="text-xl sm:text-2xl font-bold text-blue-600">{selectedSubject?.performance}</div>
              <div className="text-xs sm:text-sm text-gray-500">Performance</div>
            </div>
            <div className={`p-3 sm:p-4 rounded-lg text-center ${isDarkMode ? 'bg-slate-700' : 'bg-green-50'}`}>
              <div className="text-xl sm:text-2xl font-bold text-green-600">{selectedSubject?.students}</div>
              <div className="text-xs sm:text-sm text-gray-500">Students</div>
            </div>
            <div className={`p-3 sm:p-4 rounded-lg text-center ${isDarkMode ? 'bg-slate-700' : 'bg-purple-50'}`}>
              <div className="text-xl sm:text-2xl font-bold text-purple-600">{selectedSubject?.avgScore}%</div>
            ...
            <div className="space-y-3">
              {selectedSubject?.topics.map((topic, index) => (
                <div key={index} className="flex items-center justify-between">
                  <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{topic.name}</span>
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-medium text-sm">{topic.score}%</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className={`rounded-xl sm:rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto ${
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          <div className={`p-3 sm:p-4 rounded-lg ${
            ...
            <div className="mt-2 text-xs text-gray-500">{selectedInsight?.confidence}</div>
          </div>
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {overviewStats.map((stat, index) => {
          ...
          return (
            <div key={index} className={`p-4 sm:p-6 rounded-xl sm:rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className={`flex items-center gap-1 text-xs sm:text-sm ${stat.trend === 'up' ? 'text-green-600' : 'text-red-600'}`}>
                {stat.trend === 'up' ? <ArrowUp size={14} className="sm:w-4 sm:h-4" /> : <ArrowDown size={14} className="sm:w-4 sm:h-4" />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        <div className={`p-4 sm:p-6 rounded-xl sm:rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
          ...
          <div className="space-y-3 sm:space-y-4">
            {topPerformers.map((student, index) => (
              <div key={index} className="flex items-center gap-3 sm:gap-4">
                <div className="flex items-center justify-center w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-green-100 text-green-600 font-bold text-xs sm:text-sm">
                  {student.rank}
                </div>
                ...
                <div className="flex-1 min-w-0 text-left">
                  <h4 className={`font-medium text-xs sm:text-sm ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                    {student.name}
                  </h4>
          ...
          <div className="space-y-3 sm:space-y-4">
            {needsAttention.map((student, index) => (
              <div key={index} className="flex items-center gap-3 sm:gap-4">
                ...
                <span className={`font-semibold text-sm sm:text-base ${student.status === 'critical' ? 'text-red-600' : 'text-orange-600'}`}>
                  {student.score}
                </span>
      ...
      <div className={`p-4 sm:p-6 rounded-xl sm:rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
        ...
        <div className="h-48 sm:h-64 flex items-end justify-center gap-4 sm:gap-8">
          <div className="text-center">
            <div className="w-8 h-16 sm:w-16 sm:h-32 bg-blue-500 rounded-t mb-2"></div>
            <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Week 1</span>
          </div>
          <div className="text-center">
            <div className="w-8 h-20 sm:w-16 sm:h-40 bg-blue-500 rounded-t mb-2"></div>
            <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Week 2</span>
          </div>
          <div className="text-center">
            <div className="w-8 h-22 sm:w-16 sm:h-44 bg-blue-500 rounded-t mb-2"></div>
            <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Week 3</span>
          </div>
          <div className="text-center">
            <div className="w-8 h-24 sm:w-16 sm:h-48 bg-blue-500 rounded-t mb-2"></div>
  ...
  const renderStudents = () => (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className={`rounded-xl sm:rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
        <div className="p-4 sm:p-6">
          ...
          <div className="space-y-4">
            {filteredStudents.map((student, index) => (
              <div key={student.id} className={`p-3 sm:p-4 rounded-lg border ${
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 mb-3 sm:mb-4">
                  ...
                  <div className="flex items-center gap-3 sm:gap-4">
                    <span className={`text-xl sm:text-2xl font-bold ${getPerformanceColor(student.performance)}`}>
                      {student.overall}
                    </span>
                    <button 
                      onClick={() => setSelectedStudent(student)}
                      className="px-2 py-1 sm:px-3 sm:py-1 bg-blue-500 text-white text-xs sm:text-sm rounded-lg hover:bg-blue-600"
                    >
                      View Report
                    </button>
                  </div>
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        {subjectsData.map((subject, index) => (
          <div key={subject.id} className={`p-4 sm:p-6 rounded-xl sm:rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
            <div className="flex items-center justify-between mb-3 sm:mb-4">
              ...
              <div className={`flex items-center gap-1 text-sm ${subject.trend === 'up' ? 'text-green-600' : 'text-red-600'}`}>
                {subject.trend === 'up' ? <ArrowUp size={14} className="sm:w-4 sm:h-4" /> : <ArrowDown size={14} className="sm:w-4 sm:h-4" />}
                <span className="font-semibold text-sm">{subject.performance}</span>
              </div>
            </div>
            ...
            <div className="bg-green-100 p-2 sm:p-3 rounded-lg mb-3 sm:mb-4">
              <div className="text-xs font-medium text-green-700 mb-1">Avg. Performance</div>
              <div className="text-base sm:text-lg font-semibold text-green-700">{subject.avgScore}%</div>
            </div>
              ...
              {subject.topics.map((topic, topicIndex) => (
                <div key={topicIndex} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className={`w-8 sm:w-12 h-2 rounded-full ${
      ...
      <div className={`p-4 sm:p-6 rounded-xl sm:rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
        ...
        <div className="h-48 sm:h-64 flex items-end justify-center gap-6 sm:gap-12">
          {subjectsData.map((subject, index) => (
            <div key={index} className="text-center">
              <div className="flex gap-1 sm:gap-2 mb-3 sm:mb-4">
                <div 
                  className="w-6 sm:w-12 bg-blue-500 rounded-t"
                  style={{ height: `${subject.avgScore * 1.5}px` }}
                ></div>
                <div 
                  className="w-6 sm:w-12 bg-green-500 rounded-t"
                  style={{ height: `${(subject.students / 35 * 100) * 1.5}px` }}
                ></div>
                <div 
                  className="w-6 sm:w-12 bg-purple-500 rounded-t" 
            ...
            <table className="w-full min-w-[600px]">
              <thead>
                <tr className={`text-left border-b ${isDarkMode ? 'border-slate-600' : 'border-gray-200'}`}>
                  <th className={`pb-2 sm:pb-3 text-xs sm:text-sm font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                    SUBJECT
                  </th>
                  <th className={`pb-2 sm:pb-3 text-xs sm:text-sm font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                    STUDENTS
              ...
              <tbody>
                {subjectsData.map((subject, index) => (
                  <tr key={subject.id} className={`border-b ${isDarkMode ? 'border-slate-700' : 'border-gray-100'}`}>
                    <td className={`py-3 sm:py-4 font-medium text-sm ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                      {subject.name}
                    </td>
                    <td className={`py-3 sm:py-4 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                      {subject.students}
                    </td>
                    ...
                    <td className={`py-3 sm:py-4 font-semibold text-sm ${
                    ...
                    <td className="py-3 sm:py-4">
                      <div className="flex gap-2">
                        <button 
                          ...
                          className="p-1 text-blue-500 hover:bg-blue-50 rounded text-sm"
                        >
                          <Eye size={14} className="sm:w-4 sm:h-4" />
                        </button>
                        <button 
                          onClick={() => downloadSubjectReport(subject)}
                          className="p-1 text-green-500 hover:bg-green-50 rounded text-sm"
                        >
        ...
        {assignmentStats.map((stat, index) => (
          <div key={index} className={`p-4 sm:p-6 rounded-xl sm:rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
            ...
            <div className={`flex items-center gap-1 text-xs sm:text-sm mt-2 ${
              ...
              {stat.change.startsWith('+') ? <ArrowUp size={14} className="sm:w-4 sm:h-4" /> : <ArrowDown size={14} className="sm:w-4 sm:h-4" />}
              <span className="font-medium">{stat.change}</span>
              <span className={`ml-1 ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>
                {stat.period}
              </span>
      ...
      <div className={`p-4 sm:p-6 rounded-xl sm:rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
          ...
          {[82.3, 87.5, 87.5, 48].map((value, index) => (
            <div key={index} className="text-center">
              <div className="flex gap-1 sm:gap-2 mb-3 sm:mb-4">
                ...
                ></div>
                <div 
                  className="w-4 sm:w-8 bg-green-500 rounded-t"
                  style={{ height: `${(value + 5) * 2}px` }}
                ></div>
                <div 
                  className="w-4 sm:w-8 bg-orange-500 rounded-t"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mt-6 sm:mt-8">
          <div className="bg-blue-50 p-3 sm:p-4 rounded-lg text-center">
            <div className="text-lg sm:text-2xl font-bold text-blue-600">82.3%</div>
            <div className={`text-xs sm:text-sm ${isDarkMode ? 'text-gray-600' : 'text-gray-700'}`}>Avg Score</div>
          </div>
          <div className="bg-green-50 p-3 sm:p-4 rounded-lg text-center">
            <div className="text-lg sm:text-2xl font-bold text-green-600">87.5%</div>
            <div className={`text-xs sm:text-sm ${isDarkMode ? 'text-gray-600' : 'text-gray-700'}`}>Completion Rate</div>
          </div>
          <div className="bg-orange-50 p-3 sm:p-4 rounded-lg text-center">
            <div className="text-lg sm:text-2xl font-bold text-orange-600">67.5%</div>
            <div className={`text-xs sm:text-sm ${isDarkMode ? 'text-gray-600' : 'text-gray-700'}`}>On-time Rate</div>
          </div>
          <div className="bg-purple-50 p-3 sm:p-4 rounded-lg text-center">
            <div className="text-lg sm:text-2xl font-bold text-purple-600">48</div>
            <div className={`text-xs sm:text-sm ${isDarkMode ? 'text-gray-600' : 'text-gray-700'}`}>Total Assigned</div>
          </div>
        </div>
      </div>
            ...
            <tbody>
              {assignmentDetails.map((assignment, index) => (
                <tr key={assignment.id} className={`border-b ${isDarkMode ? 'border-slate-700' : 'border-gray-100'}`}>
                  ...
                  <td className={`py-3 sm:py-4 font-semibold text-green-600 text-sm`}>
                    {assignment.avgScore}
                  </td>
                  <td className={`py-3 sm:py-4 text-xs sm:text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
    ...
    <div className="space-y-4 sm:space-y-6">
              ...
              <div key={index} className={`p-3 sm:p-4 rounded-lg border ${
                ...
                <div className="space-y-2 text-xs">
                  ...
                  <div className="flex gap-1">
                    <span className="bg-green-100 text-green-600 px-2 py-1 rounded text-xs">High Perf.</span>
                    <span className="bg-red-100 text-red-600 px-2 py-1 rounded text-xs">Needs Focus</span>
                  </div>
                </div>
      ...
      <div className={`p-4 sm:p-6 rounded-xl sm:rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
            ...
            return (
              <div key={index} className="text-center">
                <div className="flex gap-2 sm:gap-3 mb-3 sm:mb-4">
                  ...
                  ></div>
                  <div 
                    className="w-6 sm:w-12 bg-blue-400 rounded-t"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mt-6 sm:mt-8">
          <div className="bg-green-50 p-3 sm:p-4 rounded-lg text-center">
            <div className="text-base sm:text-lg font-bold text-green-600">Best Day</div>
            <div className="text-xl sm:text-2xl font-bold text-green-600">Monday</div>
            <div className={`text-xs sm:text-sm ${isDarkMode ? 'text-gray-600' : 'text-gray-700'}`}>94.2% attendance</div>
          </div>
          <div className="bg-red-50 p-3 sm:p-4 rounded-lg text-center">
            <div className="text-base sm:text-lg font-bold text-red-600">Needs Focus</div>
            <div className="text-xl sm:text-2xl font-bold text-red-600">Friday</div>
            <div className={`text-xs sm:text-sm ${isDarkMode ? 'text-gray-600' : 'text-gray-700'}`}>89.3% attendance</div>
          </div>
        </div>
      </div>
    </div>
  ...
  const renderTrends = () => (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className={`p-4 sm:p-6 rounded-xl sm:rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mt-6 sm:mt-8">
          <div className="bg-blue-50 p-3 sm:p-4 rounded-lg">
            <div className="text-base sm:text-lg font-bold text-blue-600">Performance Trend</div>
            <div className="text-xl sm:text-2xl font-bold text-blue-600">+8.9%</div>
            <div className={`text-xs sm:text-sm ${isDarkMode ? 'text-gray-600' : 'text-gray-700'}`}>Growth since August</div>
          </div>
          <div className="bg-green-50 p-3 sm:p-4 rounded-lg">
            <div className="text-base sm:text-lg font-bold text-green-600">Attendance Trend</div>
            <div className="text-xl sm:text-2xl font-bold text-green-600">+9.8%</div>
            <div className={`text-xs sm:text-sm ${isDarkMode ? 'text-gray-600' : 'text-gray-700'}`}>Growth since August</div>
          </div>
          <div className="bg-purple-50 p-3 sm:p-4 rounded-lg">
            <div className="text-base sm:text-lg font-bold text-purple-600">Assignment Rate</div>
            <div className="text-xl sm:text-2xl font-bold text-purple-600">+8.9%</div>
            <div className={`text-xs sm:text-sm ${isDarkMode ? 'text-gray-600' : 'text-gray-700'}`}>Projected 94% in February</div>
          </div>
        </div>
      </div>
      ...
      <div className={`p-4 sm:p-6 rounded-xl sm:rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
        ...
        <div className={`text-sm mb-4 sm:mb-6 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
          Advanced insights based on data analysis
        </div>
        
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          <div className="space-y-4">
            <div className={`p-3 sm:p-4 rounded-lg border-l-4 border-blue-500 ${isDarkMode ? 'bg-slate-700' : 'bg-blue-50'}`}>
              <div className="flex items-start gap-2 sm:gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className={`text-xs sm:text-sm mb-3 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                  ...
                  <div className="flex items-center gap-3 sm:gap-4 text-xs">
                    ...
                    <button 
                      ...
                      className="text-blue-600 font-medium hover:text-blue-700 text-xs"
                    >
                      View Details →
                    </button>
                  </div>
            ...
            <div className={`p-3 sm:p-4 rounded-lg border-l-4 border-orange-500 ${isDarkMode ? 'bg-slate-700' : 'bg-orange-50'}`}>
              <div className="flex items-start gap-2 sm:gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3 sm:gap-4 text-xs">
                    ...
                    <button 
                      ...
                      className="text-orange-600 font-medium hover:text-orange-700 text-xs"
                    >
                      View Details →
                    </button>
                  </div>
          ...
          <div className="space-y-4">
            <div className={`p-3 sm:p-4 rounded-lg border-l-4 border-green-500 ${isDarkMode ? 'bg-slate-700' : 'bg-green-50'}`}>
              <div className="flex items-start gap-2 sm:gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3 sm:gap-4 text-xs">
                    ...
                    <button 
                      ...
                      className="text-green-600 font-medium hover:text-green-700 text-xs"
                    >
                      View Details →
                    </button>
                  </div>
            ...
            <div className={`p-3 sm:p-4 rounded-lg border-l-4 border-red-500 ${isDarkMode ? 'bg-slate-700' : 'bg-red-50'}`}>
              <div className="flex items-start gap-2 sm:gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3 sm:gap-4 text-xs">
                    ...
                    <button 
                      ...
                      className="text-red-600 font-medium hover:text-red-700 text-xs"
                    >
                      View Details →
                    </button>
                  </div>
        ...
        <div className="w-full h-full px-4 sm:px-6 py-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button onClick={exportToPDF} className="px-3 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 text-sm">
                Export Report
              </button>
            </div>
          </div>

          <div className={`flex overflow-x-auto gap-1 sm:gap-2 mb-6 sm:mb-8 p-1 sm:p-2 rounded-xl sm:rounded-2xl border ${
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1 sm:gap-2 px-3 py-2 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl transition-all flex-shrink-0 ${
                    activeTab === tab.id
